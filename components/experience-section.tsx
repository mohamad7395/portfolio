      ...
      <ol className="relative border-l border-border pl-6 sm:pl-8">
        {experience.map((item, i) => (
          ...
          >
            ...
            {item.location && (
              <p className="mt-1 text-lg text-muted-foreground">{item.location}</p>
            )}
            <ul className="mt-4 flex flex-col gap-2">
              {item.points.map((point) => (
                <li
                  key={point}
                  className="relative pl-5 text-lg leading-relaxed text-muted-foreground before:absolute before:left-0 before:top-2.5 before:size-1 before:rounded-full before:bg-muted-foreground/60"
                >
                  {point}
                </li>
              ))}
